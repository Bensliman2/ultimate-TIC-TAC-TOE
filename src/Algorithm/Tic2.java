...
//        static String[][] boardbackup = new String[9][9];
	...
	*/
//       public void saveboardbackup(){
//       boardbackup =this.board;
//       }
...
*/
	public void printBoard() // Printing the board
	{
        ...
        public String printfile() // Printing the board
	{
		...
		for(int k=0; k<3; k++){
			for(int i = 0; i<3; i++){//cell_num..?
				...
				for(j=0; j<3; j++){//board_num
					int board_num = j+k*3;
					int cell_num = i*3;
					file.append(" " + board[board_num][cell_num] + " | " + board[board_num][cell_num+1]  + " | " + board[board_num][cell_num+2]);
					if(j != 2) file.append(" ||");
					else {
						file.append("\n");
                                                file.append("---|---|---||---|---|---||---|---|----"+"\n");
...
*/
	public scoreStates minimax2(int depth, boolean isMaximizingPlayer, int board_num, int alpha, int beta){
		...
		if(depth == dep || emptyStates.isEmpty() || end()){
